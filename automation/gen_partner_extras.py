...
⚠️ Про рейтинг и Google (обновлено 2026): сгенерированные отзывы помечены
verified=false. build_reviews.py БОЛЬШЕ НЕ выводит AggregateRating/Review в
JSON-LD вообще (пока отзывы синтетические) — см. process_page/strip_product_rating.
Значит редакционные отзывы ПОКАЗЫВАЮТСЯ на странице (соц-доказательство), но
НЕ формируют «звёздный» rich-сниппет → нет риска ручной санкции Google за отзывы.
verified=true проставляется вручную только реальным отзывам со скрином депозита.
...
def build_reviews_payload(draft: dict, llm_items: list[dict]) -> list[dict]:
    ...
    for i, it in enumerate(llm_items):
        ...
        out.append({
            ...
            "rating": rating,
            "date": dates[i],
            # verified=false — редакционные/сидовые отзывы НЕ помечаются
            # "проверено" (нет реального игрока и скрина депозита). "проверено"
            # ставится вручную только для реальных подтверждённых отзывов.
            "verified": False,
