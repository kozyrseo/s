...
EDITORIAL: dict[str, Any] = {
    ...
    "partner_extended": {
        ...
        "tonpoker": {
            "formats": "Hold'em, PLO (кэш), MTT — игра прямо в Telegram-боте, есть веб-версия",
            "fits": (
                "Кому подходит: игрокам, которым нужны крипто-расчёты в USDT "
                "(экосистема TON) без карт и фиата, мгновенные депозиты/выводы и "
                "вход без верификации (KYC нет) прямо через аккаунт Telegram, без "
                "скачивания приложения. Трекеры и HUD не работают. Рейкбек до 30% "
                "по условиям партнёра."
            ),
        },
    },
...
def rakeback_text(p: dict) -> str:
    """Рейкбек из данных. Приоритет — явная человекочитаемая формулировка
    (rakeText / rakeLabel), где можно указать «до N%» и период выплат
    (у румов он разный: у кого еженедельно, у кого 1–3 числа месяца).
    Фолбэк — «N% еженедельно». 'none'/0/пусто → 'нет'."""
    r = p.get("rake")
    if r in (None, "", "none", 0, "0"):
        return "нет (площадка даёт только бонусы, не рейкбек)"
    explicit = (p.get("rakeText") or p.get("rakeLabel") or "").strip()
    if explicit:
        return explicit
...
def legal_status(p: dict) -> str:
    ...
    return lic or "Лицензия уточняется."
