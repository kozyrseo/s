...
(function () {
  ...
  function acceptMarkerHtml(list) {
    ...
    var title = accepted
      ...
      : (_isUk
          ? 'Партнер не обслуговує гравців з регіону: ' + label + '. Можливо, знадобиться VPN або перевірка умов руму.'
          : 'Партнёр не обслуживает игроков из региона: ' + label + '. Возможно потребуется VPN или проверка условий рума.');
    var mainText = accepted
      ? (_isUk ? 'Доступний для ' : 'Доступен для ') + label
      : (_isUk ? 'Недоступний для ' : 'Недоступен для ') + label;
    var subText = accepted
      ? (_isUk ? 'Реєстрація та гра з вашого регіону підтримуються' : 'Регистрация и игра из вашего региона поддерживаются')
      : (_isUk ? 'Партнер не обслуговує гравців з цього регіону' : 'Партнёр не обслуживает игроков из этого региона');
  ...
  function initTelegramWidget() {
    ...
    /* Язык: украинская версия сайта (/uk/ или <html lang="uk">) → UK-тексты,
       иначе русские. Один виджет обслуживает обе языковые ветки. */
    var _htmlLang = (document.documentElement.getAttribute('lang') || '').toLowerCase();
    var _isUk = _htmlLang.indexOf('uk') === 0 || /\/uk\//.test(location.pathname);
    var TG_LABEL = _isUk ? 'Питання? Напишіть нам' : 'Вопрос? Напишите нам';
    var TG_ARIA  = _isUk ? 'Написати в Telegram @kozyr_support' : 'Написать в Telegram @kozyr_support';
    var TG_CLOSE = _isUk ? 'Приховати на 24 години' : 'Скрыть на 24 часа';
    ...
    wrap.innerHTML =
      '<a class="kz-tg__btn" href="' + TG_URL + '" target="_blank" rel="noopener"' +
        ...
        '</svg>' +
        '<span class="kz-tg__label">' + TG_LABEL + '</span>' +
      '</a>' +
      '<button class="kz-tg__close" type="button" aria-label="' + TG_CLOSE + '">×</button>';
